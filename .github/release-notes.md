...
- **修复启动后主窗口不显示（进程在跑但看不到界面）**：原启动顺序在闪屏关闭后先同步加载印章库再显示主窗——库慢或为旧版本残留的损坏数据时，应用处于"零可见窗口"状态。现改为**主窗先显示、印章库后台容错加载**，并在状态栏提示进度
- **窗口可见性看门狗**：主窗若因任何原因不可见，自动强制拉起（最多 3 次）并记录日志——"隐形窗口"被直接救回或留下诊断线索
- **诊断黑洞补全**：Tk 回调异常此前在无控制台的 exe 里被静默吞掉（日志一片安静的原因之一），现在全部写入日志；启动各阶段（窗口构建/显示/进入主循环）也留有里程碑记录
- 旧版本损坏的印章库数据（非法 JSON、异常条目）不再可能阻断启动
- CI 新增「真实启动可见性」门禁：在 Windows 上以用户方式启动 exe，断言主窗口句柄出现——含全新环境与损坏印章库两个场景
