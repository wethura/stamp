...
## 状态
`部分完成，待完善` — Sprint 已归档，以下两项未实现：
1. 实例配置 JSON 持久化（关闭应用后配置丢失）
2. 模板拖拽到预览区创建实例（只有双击）
...
- `StampInstanceManager` 需接受 `doc_path` 参数
- 实现 `save()` 方法：将所有实例序列化为 JSON，写入文档同级 `.stamp-config.json`
- 实现 `load()` 方法：打开文档时读取 `.stamp-config.json`，反序列化为实例列表
- 关闭文档时自动保存，切换文档时先保存再加载
- 配置文件名格式：`.<原文件名>.stamp-config.json`（如 `.report.pdf.stamp-config.json`）
